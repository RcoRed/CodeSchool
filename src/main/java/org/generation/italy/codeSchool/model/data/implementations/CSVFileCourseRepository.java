...
public class CSVFileCourseRepository implements CourseRepository {
         ...
         for (String s:lines){                                               //ciclo per ogni riga letta
            ...
            if (courseId == id){
               Course found = new Course(courseId,trimmed[1],trimmed[2]    //creo l'oggeto passando le stringhe letta dal file
                     ,trimmed[3],Double.parseDouble(trimmed[4]),LocalDate.now());
      ...
      try{
         ...
         for(String s : lines){
            ...
            if(tokens[1].contains(part)){
               Course found = new Course(Long.parseLong(tokens[0]),tokens[1],tokens[2],tokens[3],Double.parseDouble(tokens[4]), LocalDate.parse(tokens[5]));
               courses.add(found);
            }
         }
         return courses;
   ...
   public void deleteById(long id) throws EntityNotFoundException, DataException {
      try{
         List<String> lines = Files.readAllLines(Paths.get(fileName));
         for (Iterator<String> it = lines.iterator(); it.hasNext();) { //iterator è uno dei 23 design pattern, voglio trovare un modo uniforme per iterare  su tante diverse forme di struttre dati
            String line = it.next();    //questo è quello che fa veramente un forEach
            String[] tokens = line.split(",");
            long courseId = Long.parseLong(tokens[0]);
            if (courseId == id){
               it.remove();
               try(PrintWriter pW = new PrintWriter(new FileOutputStream(fileName))) {     //serve solo a fare gestione delle risorse
                  for (String st : lines) {
                     pW.println(st);
                  }
               }
