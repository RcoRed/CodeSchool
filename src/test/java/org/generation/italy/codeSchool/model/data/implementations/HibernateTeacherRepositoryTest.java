...
import static org.generation.italy.codeSchool.model.data.implementations.HibernateTestUtils.insertEntity;
    ...
    void findWithCompetenceByLevel() {
        try{
            ...
            Optional<Competence> oc1 = tc.get(0).getCompetences()
                    .stream()
                    .filter(c -> c.getId() == co1.getId()).findFirst();
            ...
            Optional<Competence> oc2 = tc.get(0).getCompetences()
                    .stream()
                    .filter(c -> c.getId() == co2.getId()).findFirst();
